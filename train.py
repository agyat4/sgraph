...
class RateDistortionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        out["msid_loss"] = computeMSID(target, output["x_hat"])
    ...
    for i, d in enumerate(train_dataloader):
        ...
        if i % 100 == 0:
            print(
                ...
                f'\tMSID loss: {out_criterion["msid_loss"].item():.3f} |'
...
def test_epoch(epoch, test_dataloader, model, criterion):
    ...
    with torch.no_grad():
        for d in test_dataloader:
            ...
            msid_loss.update(out_criterion["msid_loss"])
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--seed", type=float,  default=42, help="Set random seed for reproducibility (default: %(default)s)"
    )
    parser.add_argument(
        "--clip_max_norm",
        default=1.0,
