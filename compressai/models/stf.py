...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        """
        B_, N, C = x.shape
        qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4).contiguous()# here changed 3 to 9 
...
class PatchEmbed(nn.Module):
    def __init__(self, patch_size=4, in_chans=7, embed_dim=96, norm_layer=None):
        super().__init__()
...
class SymmetricalTransFormer(CompressionModel):
    def __init__(self,
        ...
        self.end_conv = nn.Sequential(nn.Conv2d(embed_dim, embed_dim * patch_size ** 2, kernel_size=5, stride=1, padding=2),
                                      ...
                                      nn.Conv2d(embed_dim, 7, kernel_size=3, stride=1, padding=1), #  here 3 change to 9 as putput channel
                                      )
